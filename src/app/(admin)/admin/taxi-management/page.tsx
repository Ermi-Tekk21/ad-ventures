...
import { DriversData } from "@/utils/data/admin/driverData/driverInfo";
...
const Page: React.FC = () => {
    ...
    const handleAccept = (taxiId: number) => {
        console.log(`Accepted taxi with ID: ${taxiId}`);
    };

    const handleReject = (taxiId: number) => {
    ...
    return (
        <div className="flex flex-col items-center">
            {/* Toggle Buttons */}
            <div className="fixed py-2 z-30 pt-16 flex w-full gap-4 justify-center bg-gray-900">
                    <Button
                        className={`px-5 py-2 font-semibold rounded-md transition-all ${
                            isVisible === "active" ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-300 text-gray-700 dark:bg-gray-700 dark:text-gray-200'
                        }`}
                    ...
                    <Button
                        className={`px-5 py-2 font-semibold rounded-md transition-all ${
                            isVisible === "pending" ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-300 text-gray-700 dark:bg-gray-700 dark:text-gray-200'
                        }`}
            ...
            <ScrollArea className="h-screen w-full px-10 pt-16 rounded-lg shadow-xl  bg-gray-50 dark:bg-gray-900">
                ...
                <div className="grid grid-cols-1 pt-16 gap-6 text-gray-800 dark:text-gray-100">
                        ...
                        <div className="grid gap-4 px-6">
                            {activeTaxiData.map((taxi) => (
                                ...
                                >
                                    ...
                                    <p><span className="font-semibold">Assigned Package ID:</span> {taxi.assignedPackageId || "None"}</p>
                                    <p><span className="font-semibold">Make:</span> {taxi.taxiSpecification.make}</p>
                                    <p><span className="font-semibold">Model:</span> {taxi.taxiSpecification.model}</p>
                                    <p><span className="font-semibold">Year:</span> {taxi.taxiSpecification.year}</p>
                                    <p><span className="font-semibold">Color:</span> {taxi.taxiSpecification.color}</p>
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    {isVisible === "pending" && (
                        <div className="grid gap-4 px-6">
                            {pendingTaxiData.map((taxi) => (
                                <div
                                    key={taxi.id}
                                    className="p-6 rounded-lg bg-gray-100 dark:bg-gray-700 shadow-md transition-transform transform hover:scale-105"
                                >
